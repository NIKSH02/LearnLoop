...
const sendOtp = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    user = new User({ email, otp: otp.toString(), otpExpiry, isEmailVerified: false });
    await user.save();
...
const loginWithOtp = asyncHandler(async (req, res) => {
  const { email, otp } = req.body;
  console.log(`LOGIN OTP REQUEST: Email: ${email}, OTP: ${otp}`);
  ...
  console.log(`LOGIN OTP DEBUG: User found. Stored OTP: ${user.otp}, Received OTP: ${otp}, OTP Expiry: ${user.otpExpiry}, Current Time: ${Date.now()}`);
  
  if (!user.otp) {
    console.log(`LOGIN OTP FAIL: No OTP stored for ${email}`);
